...
export function AllCases() {
  ...
  const { data, isLoading } = useAllCases(params);
  const cases = data?.items ?? [];

  // Redirect non-superusers away (belt-and-suspenders; App.tsx also guards this route)
  if (!isSuperuser) {
    void navigate('/cases/my', { replace: true });
    return null;
  }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">{t('cases.allCasesTitle')}</h1>
      </div>
